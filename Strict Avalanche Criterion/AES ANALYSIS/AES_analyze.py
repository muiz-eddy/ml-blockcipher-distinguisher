...
#BIT DIFFERENCE
def solve(A, B):
    """ Generate count for the bit difference 
    
    For every bit difference count is incremented by 1 
...
def flip(s):
    pos = secrets.SystemRandom().choice(range(len(s)))
    r = list(s)
...
nums = input("Text Size: ")
hypo = input("Hypo File name:")
...
for i in range(10):
    ...
    f = open("E:\\Data\\AES\\TextFile\\" + user_choice + " " + str(i+1) + ".txt", "w")
    f.write('\n'.join(pt_list))
    f.close()
...
for i in range(10):
    file = open("E:\\Data\\AES\\TextFile\\" + user_choice + " " + str(i+1) + ".txt", 'r')
    ...
    for j in range(len(original)):
        key = bytes.fromhex(keys[i])
        pt1 = bytes.fromhex(original[j])
        pt2 = bytes.fromhex(flipped[j])
        ct = AESCipher(key)
        ciphertext1 = ct.encrypt(pt1)   
        ciphertext1 = ciphertext1.hex()
        result_ori_AES.append(ciphertext1)
        ciphertext2 = ct.encrypt(pt2)
        ciphertext2 = ciphertext2.hex()
        result_ori_AES.append(ciphertext2)
        
    with open("E:\\Data\\AES\\ctfile_AES\\" + user_choice + " " + str(i + 1) + ".txt", "w") as ct2:
            ct2.write("\n".join(result_ori_AES))
...
for i in range(10):
    file = open("E:\\Data\\AES\\ctfile_AES\\" + user_choice + " " + str(i+1) + ".txt", "r")
    ...
    pd_a.append(observed_value_s)       
    
for i in range(len(pd_a)):
    chi,chi2= chisquare(pd_a[i],expected)
    chis.append(chi2)
    
for i in range(10):
    header = ['Bins', 'Total Bits Change (observed)', 'Expected (Binomial (n = 128, p = 0.5)', 'Chisquare']
    
    rows = zip(h,pd_a[i],expected,chis[i])
    with open("E:\\Data\\AES\\result_AES\\" + user_choice + str(i + 1) + ".csv", 'w', encoding='UTF8') as f:
        writer = csv.writer(f)
...
print("AES Chisquare Average: ", chiavg)
print("Critical Value: ", dof)   

# rows2 = zip(nums,str(chiavg),str(time.process_time() - start))

with open("E:\\Data\\AES\\hypothesis\\" + hypo + ".csv", 'a', encoding='UTF8',newline="") as ct2:
...
# plt.bar(r_values, chis[2]) 
# from scipy.stats import chisquare

# chi = chisquare(pd_s[0])
# print("AES Chisquare Average: ", chi)           
